    ...
    def _load_data(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                #self.image_file.append(self.read_image(img_path))
        ...
        for phase, indices in split_profiles.items():
            for _idx in indices:
                ...
                for file_name in os.listdir(img_folder):
                    ...
                    last_label = self.encode_multi_class(mask_label, gender_label, age_label)
                    if last_label == 0:
                        ...
                        if np.random.randint(8) != 0:
                            continue
                    elif last_label in [9, 10, 12, 15, 16]:
...
class Undersampling(MaskBaseDataset):
        ...
        for phase, indices in split_profiles.items():
            for _idx in indices:
                ...
                for file_name in os.listdir(img_folder):
                    ...
                    gender_label = GenderLabels.from_str(gender)
                    age_label = AgeLabels.from_number(age)
                    last_label = int(mask_label)*6 + int(gender_label)*3 + int(age_label)
                    if last_label == 0:
...
# class AgeSplitByProfileDataset(MaskBaseDataset):
